...
namespace CGAL
{
  ...
  namespace Euler
  {
                ...
                Graph &g)
    {
      ...
      halfedge_descriptor hop = opposite(h, g), hprev = prev(hop, g), gprev = prev(h, g), hnext = next(hop, g), gnext = next(h, g);
      vertex_descriptor v_to_remove = target(hop, g), v = target(h, g);
              ...
              Graph &g)
    {
      ...
      CGAL_assertion_code(std::size_t termination_count = 0;) do
      {
               ...
               Graph &g)
    {
      ...
      CGAL_assertion_code(std::size_t termination_count = 0;) if (next(h, g) != i)
      {
    ...
    bool can_add_face(const VertexRange &vrange, const PMesh &sm)
    {
      ...
      for (std::size_t i = 0; i < N; ++i)
      {
        ...
        if (prev_hd != boost::graph_traits<PMesh>::null_halfedge())
        {
          ...
          if (next_hd == boost::graph_traits<PMesh>::null_halfedge())
            continue;
          ...
          {
            halfedge_around_vertex = opposite(next(halfedge_around_vertex, sm), sm);
            if (is_border(opposite(halfedge_around_vertex, sm), sm))
              break;
          } while (halfedge_around_vertex != prev_hd);
    ...
    add_face(const VertexRange &vr, Graph &g)
    {
      ...
      {
        if (!internal::is_isolated(vertices[i], g) && !is_border(vertices[i], g))
          return boost::graph_traits<Graph>::null_face();
    ...
    void add_faces(const RangeofVertexRange &faces_to_add, PolygonMesh &pm)
    {
      ...
      auto add_new_edge = [&pm](vertex_descriptor v1, vertex_descriptor v2)
      {
        ...
        if (halfedge(v1, pm) == GT::null_halfedge())
          set_halfedge(v1, v2v1, pm);
        if (halfedge(v2, pm) == GT::null_halfedge())
          set_halfedge(v2, v1v2, pm);
        ...
        for (halfedge_descriptor h : former_border_hedges)
        {
          ...
          if (!std::binary_search(former_border_hedges.begin(), former_border_hedges.end(), nh))
          {
            do
            {
              ...
              if (candidate == h)
                break; // stop condition for a vertex that will stay on the boundary after the operation
              if (std::binary_search(former_border_hedges.begin(), former_border_hedges.end(), nh))
              {
          ...
          if (!std::binary_search(former_border_hedges.begin(), former_border_hedges.end(), ph))
          {
            do
            {
              ...
              if (candidate == h)
                break;
              ; // stop condition for a vertex that will stay on the boundary after the operation
              if (std::binary_search(former_border_hedges.begin(), former_border_hedges.end(), ph))
              {
                // linking halfedges that will no longer be on the boundary
                set_next(ph, h, pm);
      ...
      auto get_hedge = [&pm, &add_new_edge, &new_border_halfedges, &outgoing_hedges, &vid](vertex_descriptor v1, vertex_descriptor v2)
      {
        ...
        typename Halfedges::const_iterator it_find =
            std::lower_bound(v1_outgoing_hedges.begin(),
                             ...
                             v2, [&pm](halfedge_descriptor h, vertex_descriptor v)
                             { return target(h, pm) < v; });
    ...
    collapse_edge(typename boost::graph_traits<Graph>::edge_descriptor v0v1,
                  Graph &g, EdgeIsConstrainedMap Edge_is_constrained_map)
    {
      typedef boost::graph_traits<Graph> Traits;
      typedef typename Traits::vertex_descriptor vertex_descriptor;
      typedef typename Traits::halfedge_descriptor halfedge_descriptor;
      ...
      {
        if (face(edges_to_erase[0], g) == face(edges_to_erase[1], g) && (!is_border(edges_to_erase[0], g)))
        {
          // the vertex is of valence 3 and we simply need to remove the vertex
          // and its indicent edges
          halfedge_descriptor edge =
              next(edges_to_erase[0], g) == edges_to_erase[1] ? edges_to_erase[0] : edges_to_erase[1];
          if (target(edge, g) != p)
          {
                                ...
                                const Graph &g)
    {
        ...
        if (v0_k != v0_v1)
        {
          ...
          for (boost::tie(eb2, ee2) = halfedges_around_source(k, g); eb2 != ee2; ++eb2)
          {
            ...
            if (target(k_v1, g) == v1)
            {
              ...
              bool lIsFace = (vL == k && (!is_border(v0_v1, g))) || (vR == k && (!is_border(v1_v0, g)));
      ...
      if (!is_border(v0_v1, g) && is_border(opposite(next(v0_v1, g), g), g) && is_border(opposite(prev(v0_v1, g), g), g))
        return false;
      if (!is_border(v1_v0, g) && is_border(opposite(next(v1_v0, g), g), g) && is_border(opposite(prev(v1_v0, g), g), g))
        return false;
