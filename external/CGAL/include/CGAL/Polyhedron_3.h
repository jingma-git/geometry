...
namespace CGAL
{
    ...
    public:
        ...
        {
            CGAL_precondition(this != &*(this->next()->opposite()));
            return (this != &*(this->next()->opposite()->next()->opposite()) && this == &*(this->next()->opposite()->next()->opposite()->next()->opposite()));
        }
    ...
    public:
        ...
        is_triangle(Halfedge_const_handle h1) const
        {
            ...
            if (h1->opposite()->next() != h3->opposite())
                return false;
            if (h2->opposite()->next() != h1->opposite())
                return false;
            if (h3->opposite()->next() != h2->opposite())
                return false;
            // The facet is a triangle.
            if (h1->next()->next()->next() != h1)
                return false;

            if (check_tag(Supports_halfedge_face()) && !h1->is_border_edge())
        ...
        is_tetrahedron(Halfedge_const_handle h1) const
        {
            ...
            if (h4->opposite()->next() != h3->opposite())
                return false;
            if (h5->opposite()->next() != h1->opposite())
                return false;
            if (h6->opposite()->next() != h2->opposite())
                return false;
            // three edges at v4.
            if (h4->next()->opposite() != h5)
                return false;
            if (h5->next()->opposite() != h6)
                return false;
            if (h6->next()->opposite() != h4)
                return false;
            // All facets are triangles.
            if (h1->next()->next()->next() != h1)
                return false;
            if (h4->next()->next()->next() != h4)
                return false;
            if (h5->next()->next()->next() != h5)
                return false;
            if (h6->next()->next()->next() != h6)
                return false;
        ...
        Halfedge_handle join_facet(Halfedge_handle h)
        {
            ...
            HalfedgeDS_decorator<HDS> D(hds_);
            CGAL_precondition(circulator_size(h->vertex_begin()) >= size_type(3));
            CGAL_precondition(circulator_size(h->opposite()->vertex_begin()) >= size_type(3));
        ...
        Halfedge_handle join_vertex(Halfedge_handle h)
        {
            ...
            HalfedgeDS_decorator<HDS> D(hds_);
            CGAL_precondition(circulator_size(h->facet_begin()) >= size_type(4));
            CGAL_precondition(circulator_size(h->opposite()->facet_begin()) >= size_type(4));
        ...
        {
            HalfedgeDS_decorator<HDS> D(hds_);
            CGAL_assertion(circulator_size(h->facet_begin()) >= size_type(3));
        ...
        Halfedge_handle join_loop(Halfedge_handle h, Halfedge_handle g)
        {
            ...
            CGAL_precondition(circulator_size(h->facet_begin()) >= size_type(3));
            CGAL_precondition(circulator_size(h->facet_begin()) == circulator_size(g->facet_begin()));
        ...
        double avg_edge_len() const
        {
            ...
            {
                auto v0 = it->opposite()->vertex(); // source
                auto v1 = it->vertex();             // target
                double l = std::sqrt((v1->point() - v0->point()).squared_length());
                sum += l;
                ++count;
            }
        ...
        void normalize()
        {
            ...
            for (Vertex_iterator it = vertices_begin(); it != vertices_end(); ++it)
            {
                ...
                it->point() = Point_3(p.x() * scl, p.y() * scl, p.z() * scl);
            }
        }
    ...
    protected: // Supports_face_plane
        ...
        {
            typename Traits::Construct_opposite_plane_3 opp = traits().construct_opposite_plane_3_object();
