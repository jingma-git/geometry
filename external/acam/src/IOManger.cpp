...
#define LINE_LEN 4096
        ...
        bool OBJReader::loadMeshFromOBJ(std::istream &_in, PolyMesh *mesh, IOOptions &opt)
        {
            ...
            while (_in && !_in.eof())
            {
                ...
                stream.str(line);
                stream.clear(); // clear����������ݣ��������������Ϣ������fail��bad��Щ����־
                stream >> keyWrd;
                ...
                else if (keyWrd == "vt")
                {
                    ...
                    {
                        std::ostringstream error_info;
                        error_info << "[OBJReader] : Only single 2D or 3D texture coordinate per vertex"
                                   << "allowed!" << std::endl;
                        Massage::Error(error_info.str());
                        return false;
                    }
        ...
        bool OFFReader::loadMeshFromOFF(std::istream &_in, PolyMesh *mesh, IOOptions &opt)
        {
            ...
            if (options_.vert_have_normal && user_options.vert_have_normal)
                opt.vert_have_normal;
            if (options_.vert_have_texture && user_options.vert_have_texture)
                opt.vert_have_texture;
            if (options_.vert_have_color && user_options.vert_have_color)
                opt.vert_have_color;
            if (options_.face_has_color && user_options.face_has_color)
                opt.face_has_color;
            ...
            for (i = 0; i < nV && !_in.eof(); ++i)
            {
                ...
                if (options_.vert_have_color)
                {
                    ...
                    case 1:
                        stream >> trash;
                        break; // one int (isn't handled atm)
                    case 2:
                        stream >> trash;
                        stream >> trash;
                        break; // corrupt format (ignore)
                    // rgb int
                    case 3:
                        stream >> c0;
                        stream >> c1;
        ...
        bool OFFWriter::write(std::ostream &_os, PolyMesh *mesh, IOOptions _opt, std::streamsize _precision) const
        {
            ...
            if (_opt.vert_have_texture)
                _os << "ST";
            if (_opt.vert_have_color || _opt.face_has_color)
                _os << "C";
            if (_opt.vert_have_normal)
                _os << "N";
            _os << "OFF";
