...
namespace acamcad
{
	namespace polymesh
	{
		...
		MVert *PolyMesh::newVertex()
		{
			...
			new (w_ptr) MVert(0, 0, 0); // placement new, place MVert's content in address w_ptr
		...
		MPolyFace *PolyMesh::addPolyFace(std::vector<MHalfedge *> &he_loop)
		{
			...
			face->update(); // update face's vertex num and normal

			// set halfedge's pointer to face
			for (size_t i = 0; i < he_size; i++)
			{
		...
		MPolyFace *PolyMesh::addPolyFace(std::vector<MVert *> &v_loop)
		{
			...
			for (size_t i = 0; i < v_size; i++)
			{
				if (!isBoundary(v_loop[i]))
				{
					...
					error_info << "[AddFace] : complex vertex " << v_loop[0]->index() << " " << v_loop[1]->index() << " " << v_loop[2]->index() << std::endl;
				...
				if (edge != nullptr && !isBoundary(edge))
				{
					...
					std::ostringstream error_info;
					error_info << "[AddFace] : complex edge " << v_loop[0]->index() << " " << v_loop[1]->index() << " " << v_loop[2]->index() << std::endl;
					Massage::Error(error_info.str());
				}
			}
#endif // !NDEBUG
			...
			MHalfedge *he_ip, *he_in; // in previous, in next
			MHalfedge *he_op, *he_on; // out previous, out next
			MHalfedge *he_bp, *he_bn; // boundary previous, boundary next
			...
			for (size_t i = 0; i < v_size; i++)
			{
				...
				if (!is_edge_new[i] && !is_edge_new[ii])
				{
					...
					if (he_ip->next() != he_in)
					{
						...
						patch_end->setNext(he_bn);
						he_bn->setPrev(patch_end);
						he_ip->setNext(he_in);
						he_in->setPrev(he_ip);
					}
				}
			...
			for (size_t i = 0; i < v_size; i++)
			{
				...
				MVert *vh = v_loop[i1];
				he_ip = vec_edges[i];  // in previous
				he_in = vec_edges[i1]; // in next

				assert(he_ip != nullptr);
				assert(he_in != nullptr);

				size_t id = 0;
				if (is_edge_new[i]) // 0 | 1 = 1: edge_i is new, edge_i1 is old
					id |= 1;
				if (is_edge_new[i1]) // 0 | 2 = 2: edge_i is old, edge_i1 is new
					id |= 2;		 // 1 | 2 = 3: edge_i is new, edge_i1 is new
		...
		MVert *PolyMesh::splitEdgeMakeVertex(MEdge *edge)
		{
			...
			het0->setNext(he0_next);
			he0_next->setPrev(het0);
			he0->setNext(het0);
			het0->setPrev(he0);

			het1->setNext(he1);
			he1->setPrev(het1);
			het1->setPrev(he1_prev);
			he1_prev->setNext(het1);
		...
		MEdge *PolyMesh::jointEdgeRemoveVertex(MVert *vert)
		{
			...
			he0->setPrev(he2_prev);
			he2_prev->setNext(he0);
			he1->setNext(he3_next);
			he3_next->setPrev(he1);
			e0->updateVert();

			if (p0 != nullptr && p0->halfEdge() == he2)
				p0->setHalfedge(he0);
			if (p1 != nullptr && p1->halfEdge() == he3)
				p1->setHalfedge(he1);
		...
		MEdge *PolyMesh::mergeEdge(MEdge *edge0, MEdge *edge1)
		{
			...
			MVert *v00 = edge0->getVert(0);
			MVert *v01 = edge0->getVert(1);
			MVert *v10 = edge1->getVert(0);
			MVert *v11 = edge1->getVert(1);
			MVert *ev = nullptr;
			if (v10 == v00 || v10 == v01)
			{
				ev = v10;
		...
		MPolyFace *PolyMesh::jointFaceRemoveEdge(MPolyFace *f0, MPolyFace *f1, MEdge *edge)
		{
			...
			he0_pre->setNext(he1_next);
			he1_next->setPrev(he0_pre);
			he1_pre->setNext(he0_next);
			he0_next->setPrev(he1_pre);
		...
		MEdge *PolyMesh::splitFaceMakeEdge(MPolyFace *face, MVert *v0, MVert *v1)
		{
			...
			he0_new->setNext(he0);
			he0->setPrev(he0_new);
			he0_new->setPrev(he1_pre);
			he1_pre->setNext(he0_new);

			he1_new->setNext(he1);
			he1->setPrev(he1_new);
			he1_new->setPrev(he0_pre);
			he0_pre->setNext(he1_new);

			he0_new->setPolygon(face);
			face->setHalfedge(he0);
		...
		MEdge *PolyMesh::splitQuadrilateralTriangle(MPolyFace *face, bool isqulaty)
		{
			...
			MHalfedge *he0 = face->halfEdge();
			MVector3 vec0 = he0->tangent();
			MHalfedge *he1 = he0->next();
			MVector3 vec1 = he0->tangent();
			MHalfedge *he2 = he1->next();
			MVector3 vec2 = he0->tangent();
			MHalfedge *he3 = he2->next();
			MVector3 vec3 = he0->tangent();
		...
		bool PolyMesh::weldVertex(MVert *v_ori, MVert *v_tar, double tolerance)
		{
			...
			std::vector<MEdge *> edge_pair0;
			edge_pair0.clear();
			std::vector<MEdge *> edge_pair1;
			edge_pair1.clear();
		...
		bool PolyMesh::weldEdge(MEdge *e0, MEdge *e1, double tolerance)
		{
			...
			MHalfedge *he00_pre = he00->prev();
			MHalfedge *he00_next = he00->next();
			MHalfedge *he01_pre = he01->prev();
			MHalfedge *he01_next = he01->next();
			MHalfedge *he10_pre = he10->prev();
			MHalfedge *he10_next = he10->next();
			MHalfedge *he11_pre = he11->prev();
			MHalfedge *he11_next = he11->next();

			MPolyFace *f0 = he01->polygon();
			MPolyFace *f1 = he11->polygon();

			he10_pre->setNext(he00_next);
			he00_next->setPrev(he10_pre);
			he10_next->setPrev(he00_pre);
			he00_pre->setNext(he10_next);

			he10->setPrev(he01_pre);
			he01_pre->setNext(he10);
			he10->setNext(he01_next);
			he01_next->setPrev(he10);
		...
		bool PolyMesh::separateSingleVert(MVert *vert, std::vector<MVert *> &new_vert_list)
		{
			...
			new_vert_list.clear();
			new_vert_list.push_back(vert);

			std::vector<MHalfedge *> vhe_begin_list;
			vhe_begin_list.clear();
			std::vector<MHalfedge *> vhe_end_list;
			vhe_end_list.clear();
			...
			for (size_t i = 0; i < vhe_begin_list.size(); i++)
			{
				...
				he_begin->setPrev(he_end);
				he_end->setNext(he_begin);
			}
		...
		bool PolyMesh::separateSingleEdge(MEdge *edge, std::vector<MEdge *> &new_edge_list)
		{
			...
			he0_new->setPair(he1_new);
			he1_new->setPair(he0_new);
			he0_new->setVert(ev0);
			he1_new->setVert(ev1);
			he0_new->setEdge(e_new);
			he1_new->setEdge(e_new);
			e_new->setHalfedge(he0_new);

			he0_new->setPolygon(nullptr);
			he1_new->setPolygon(ef1);
			if (ef1->halfEdge() == he1)
			...
			he1_new->setPrev(he1_prev);
			he1_prev->setNext(he1_new);
			he1_new->setNext(he1_next);
			he1_next->setPrev(he1_new);
		...
		void PolyMesh::reverse_mesh()
		{
			...
			std::vector<MHalfedge *> he_prev_list;
			he_prev_list.clear();
			he_prev_list.reserve(half_edges_.size());
			std::vector<MHalfedge *> he_next_list;
			he_next_list.clear();
			he_next_list.reserve(half_edges_.size());
			std::vector<MVert *> he_tovert;
			he_tovert.clear();
			he_tovert.reserve(half_edges_.size());
		...
		{
			std::vector<MHalfedge *> fh_list;
			fh_list.clear();
			fh_list.reserve(face->PolyNum());
			std::vector<MHalfedge *> fh_prev_list;
			fh_prev_list.clear();
			fh_prev_list.reserve(face->PolyNum());
			std::vector<MHalfedge *> fh_next_list;
			fh_next_list.clear();
			fh_next_list.reserve(face->PolyNum());
			std::vector<MVert *> fh_tovert;
			fh_tovert.clear();
			fh_tovert.reserve(face->PolyNum());
		...
		bool PolyMesh::reverseIsolatedFace(MPolyFace *face)
		{
			...
			std::vector<MHalfedge *> fhp_list;
			fhp_list.clear();
			fhp_list.reserve(face->PolyNum());
			std::vector<MHalfedge *> fhp_prev_list;
			fhp_prev_list.clear();
			fhp_prev_list.reserve(face->PolyNum());
			std::vector<MHalfedge *> fhp_next_list;
			fhp_next_list.clear();
			fhp_next_list.reserve(face->PolyNum());
			std::vector<MVert *> fhp_tovert;
			fhp_tovert.clear();
			fhp_tovert.reserve(face->PolyNum());
		...
		MVert *PolyMesh::splitEdgeTriangle(MEdge *edge)
		{
			...
			if (!he0->isBoundary())
			{
				...
				if (f0->PolyNum() != 3)
				{
					Massage::Error("Face must triangle");
					return nullptr;
				}
			}
			if (!he1->isBoundary())
			...
			MVert *vt = newVertex();
			MPoint3 p0 = v0->position();
			MPoint3 p1 = v1->position();
		...
		void PolyMesh::collpaseLoop(MHalfedge *he)
		{
			...
			he1->setNext(hf0p_next);
			hf0p_next->setPrev(he1);
			hf0p_prev->setNext(he1);
			he1->setPrev(hf0p_prev);

			he1->setPolygon(f1);

			v0->setHalfedge(he1);
			v0->adjustOutgoingHalfedge();
			v1->setHalfedge(he1_p);
			v1->adjustOutgoingHalfedge();
		...
		bool PolyMesh::is_collapse_ok(MHalfedge *v0v1)
		{
			...
			std::vector<MVert *> intersect_vector;
			intersect_vector.clear();
		...
		void PolyMesh::flipEdgeTriangle(MEdge *edge)
		{
			...
			he0->setNext(he1_nn);
			he1_nn->setPrev(he0);
			he1_nn->setNext(he0_n);
			he0_n->setPrev(he1_nn);
			he0_n->setNext(he0);
			he0->setPrev(he0_n);

			he1->setNext(he0_nn);
			he0_nn->setPrev(he1);
			he0_nn->setNext(he1_n);
			he1_n->setPrev(he0_nn);
			he1_n->setNext(he1);
			he1->setPrev(he1_n);
		...
		{
			if (face != nullptr)
				return face->getFaceCenter();
			else
				return MPoint3(0, 0, 0);
		}

		MPoint3 PolyMesh::calculatEdgeCenter(MEdge *edge)
		{
			if (edge != nullptr)
				return edge->getCenter();
			else
				return MPoint3(0, 0, 0);
		}
